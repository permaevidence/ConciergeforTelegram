            ...
            **Turn start time**: \(currentDateTime) (\(timezone))
            ...
            if claudeCodeDocumentModeEnabled {
                ...
                **\\(codeCLIProviderName) as your Local Sub-Agent / Execution Engine**:
                ...
                - When reusing an existing project, call view_project_history for that same project_id once per turn before the first run_claude_code call. Do not repeat it again in the same turn for the same project unless the history was not loaded successfully.
